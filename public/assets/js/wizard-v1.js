...
function buildProjectPreview(e) {
    ...
    $("#project-preview-type").find(".media-flex-center").remove(),
        ...
        "" != project.name ? $("#project-preview-title span").html(project.name) : $("#project-preview-title span").html("Project Name Goes Here"),
        "" != project.description ? $("#project-preview-description").html(project.description) : $("#project-preview-description").html("You didn't enter any description. You can edit it anytime by clicking on the small edit icon."),
        null != project.customer.logoUrl || "" != project.customer.name
            ...
            : ($("#project-preview-customer-block").addClass("is-hidden"), $("#project-preview-customer-placeholder").removeClass("is-hidden")),
        $("#project-preview-budget span").html(project.info.budget),
        "" != project.info.date ? $("#project-preview-date span").html(project.info.date) : $("#project-preview-date span").html("Unset"),
    ...
    if (($("#project-preview-tools .media-list-item").remove(), project.tools.length > 0)) {
        $("#project-preview-tools-placeholder").addClass("is-hidden"), $("#project-preview-tools").removeClass("is-hidden");
    ...
    } else $("#project-preview-tools").addClass("is-hidden"), $("#project-preview-tools-placeholder").removeClass("is-hidden");
    console.log(project);
}
...
function goToStep(e) {
    (currentStep = e), $(".wizard-v1-wrapper .inner-wrapper").removeClass("is-active"), $("#wizard-step-" + e).addClass("is-active");
    var t = '\n        <span class="title-wrap">Step ' + (e + 1) + ": <span>" + $(".inner-wrapper.is-active").attr("data-step-title") + "</span></span>\n    ";
    $(".is-wizard-title").html(t),
        ...
        0 == currentStep
            ...
            : 7 == currentStep && (handleProgress(100), $(".wizard-buttons").removeClass("is-active"), $(".wizard-dropdown").addClass("is-hidden")),
    ...
    $(".permission-level-inner")
        ...
        .on("click", function () {
            ...
            e.closest(".permission-levels").find(".progress").val(t),
                ...
                e.closest(".permission-level").prevAll().find(".permission-level-inner").addClass("is-active"),
                e.closest(".permission-level").nextAll().find(".permission-level-inner").removeClass("is-active");
        });
}
function removeMember() {
    $(".invited-member .cancel-button")
        .off()
        .on("click", function () {
            $(this).closest(".invited-member").remove(), 0 === $(".invited-member").length && $(".empty-wrap").removeClass("is-hidden");
        });
}
function addMember(e, t, i) {
    ...
    $(".empty-wrap").addClass("is-hidden"), $("#invited-member-" + e).length ? notyf.open({ type: "warning", message: "You already Invited " + i }) : ($(".members-list").append(s), removeMember(), initPermissions());
}
function intializeWizard(){
    $(".type-select-button").on("click", function () {
        ...
        $(".wizard-button-next").on("click", function () {
            var e = $(this);
            if ((e.addClass("is-loading"), 1 == currentStep)) (project.name = $("#project-name").val()), (project.description = $("#project-description").val());
            ...
            else if (3 == currentStep) project.info.attachments = $(".preview-box").length;
    ...
    var e = document.querySelectorAll(".form-datepicker"),
        t = [];
    for (var i = 0; i < e.length; i++) t[i] = new Pikaday({ field: e[i], firstDay: 1, format: "MMM D, YYYY", onSelect: function () {} });
    var s = {
        url: "http://localhost:3000/assets/data/companies.json",
        getValue: "name",
        template: {
            type: "custom",
            method: function (e, t) {
        ...
        list: {
            ...
            onChooseEvent: function () {
                ...
                $("#customer-logo").attr("src", e.pic), $("#customer-name").html(e.name), $("#customer-location").html(e.location), $(".project-customer").find(".field, .media-flex-center").toggleClass("is-hidden");
            },
        },
    };
    $("#customers-search").easyAutocomplete(s),
        $("#remove-customer").on("click", function () {
            $("#customers-search").val(""), $(".project-customer").find(".field, .media-flex-center").toggleClass("is-hidden");
        }),
        $(".project-budget .budget-item-inner").on("click", function () {
            $(this).closest(".project-budget").find(".budget-item-inner").removeClass("is-active"), $(this).addClass("is-active");
        }),
        ...
        var n = {
            ...
            list: {
                ...
                onChooseEvent: function () {
                    ...
                    addMember($("#add-member").getSelectedItemData().id, t, e), $("#add-member").val(""), 1;
                },
            },
        };
